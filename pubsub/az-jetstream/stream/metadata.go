...
type metadata struct {
	...
	internalDeliverPolicy jetstream.DeliverPolicy `mapstructure:"-"`
	AckPolicy             string                  `mapstructure:"ackPolicy"`
	internalAckPolicy     jetstream.AckPolicy     `mapstructure:"-"`
	Domain                string                  `mapstructure:"domain"`
	APIPrefix             string                  `mapstructure:"apiPrefix"`
	Subjects              []string                `mapstructure:"subjects"`
	MaxAge                time.Duration           `mapstructure:"maxAge"`
	InactiveThreshold     time.Duration           `mapstructure:"heartbeat"`
...
func parseMetadata(psm pubsub.Metadata) (metadata, error) {
	...
	if m.Name == "" {
		m.Name = "azarc.io - pubsub.jetstream"
	}
	...
	case "all", "":
		m.internalDeliverPolicy = jetstream.DeliverAllPolicy
	case "last":
		m.internalDeliverPolicy = jetstream.DeliverLastPolicy
	case "new":
		m.internalDeliverPolicy = jetstream.DeliverNewPolicy
	case "sequence":
		m.internalDeliverPolicy = jetstream.DeliverByStartSequencePolicy
	case "time":
		m.internalDeliverPolicy = jetstream.DeliverByStartTimePolicy
	default:
	...
	switch m.AckPolicy {
	case "explicit":
		m.internalAckPolicy = jetstream.AckExplicitPolicy
	case "all":
		m.internalAckPolicy = jetstream.AckAllPolicy
	case "none":
		m.internalAckPolicy = jetstream.AckNonePolicy
	default:
		m.internalAckPolicy = jetstream.AckExplicitPolicy
	}
